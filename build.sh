...
# Maestro MCP Server Build Script
...
print_help() {
    echo -e "${BLUE}Maestro MCP Server Build Script${NC}"
    echo ""
...
build_server() {
    print_header "Building Maestro MCP Server..."
    ...
    LDFLAGS="-X github.com/maximilien/maestro-mcp/src/pkg/config.Version=$VERSION"
    LDFLAGS="$LDFLAGS -X github.com/maximilien/maestro-mcp/src/pkg/config.GitCommit=$GIT_COMMIT"
    LDFLAGS="$LDFLAGS -X github.com/maximilien/maestro-mcp/src/pkg/config.BuildTime=$BUILD_TIME"
    
    if ! go build -ldflags "$LDFLAGS" -o bin/maestro-mcp ./src/main.go; then
        print_error "MCP server build failed"
        exit 1
    fi
    
    # Make binary executable
    chmod +x bin/maestro-mcp
    
    print_status "✅ MCP server built successfully!"
    print_status "   • Binary: bin/maestro-mcp"
    print_status "   • Size: $(stat -f%z bin/maestro-mcp 2>/dev/null | numfmt --to=iec || echo "unknown")"
}
...
Maestro MCP Server Build Information
=====================================
...
MCP Server Build:
$(if [ -f "bin/maestro-mcp" ]; then
    echo "  Status: Built"
    echo "  Version: $VERSION"
    echo "  Go Version: $(go version 2>/dev/null || echo "unknown")"
    echo "  Binary: bin/maestro-mcp ($(stat -f%z bin/maestro-mcp 2>/dev/null | numfmt --to=iec || echo "unknown"))"
else
...
Features:
  Vector Database Support: Milvus, Weaviate, Mock
  MCP Tools: create_vector_database, write_document, query, list_documents, etc.
...
build_all() {
    ...
    echo "====================================="
    print_status "Maestro MCP Server has been built successfully!"
    echo ""
    print_status "Built components:"
    print_status "  • MCP Server Binary: bin/maestro-mcp"
    print_status "  • Build Info: bin/build-info.txt"
    echo ""
    print_status "Next steps:"
    print_status "  1. Run './bin/maestro-mcp' to start the server"
