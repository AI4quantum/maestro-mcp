...
print_help() {
    echo -e "${BLUE}Maestro MCP Server Test Suite${NC}"
    echo ""
...
run_integration_tests() {
    ...
    # Run Milvus integration tests if configured
    if [ -n "$MAESTRO_MCP_VECTOR_DB_MILVUS_HOST" ] && [ "$MAESTRO_MCP_VECTOR_DB_MILVUS_HOST" != "localhost" ]; then
        print_status "Running Milvus integration tests..."
        if go test -v -timeout=30s ./tests/... -run="TestMilvus"; then
            print_success "Milvus integration tests passed!"
        else
            print_warning "Milvus integration tests failed"
        fi
    else
        print_warning "Skipping Milvus integration tests - no credentials provided"
        print_status "Set MAESTRO_MCP_VECTOR_DB_MILVUS_HOST to run Milvus tests"
    fi
    
    # Run Weaviate integration tests if configured
    if [ -n "$MAESTRO_MCP_VECTOR_DB_WEAVIATE_URL" ] && [ "$MAESTRO_MCP_VECTOR_DB_WEAVIATE_URL" != "http://localhost:8080" ]; then
        ...
        print_status "Set MAESTRO_MCP_VECTOR_DB_WEAVIATE_URL to run Weaviate tests"
    fi
}
